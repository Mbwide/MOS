...
/* 信号量列表 */
extern mos_list_t g_mos_shell_ipc_list;
...
mos_sync_t * mos_ipc_sync_creat(void)
{
	...
	/* 初始化信号量所属shell列表 */
	mos_list_init(&(to_creat_sync_ipc->sync_ipc.shell_list));
    /* 将同步信号量插入shell列表 */
    mos_list_tail_insert(&g_mos_shell_ipc_list, &(to_creat_sync_ipc->sync_ipc.shell_list));
#endif	
...
#if (MOS_CONFIG_USE_SHELL == YES)
	mos_list_init(&(to_creat_mutex_ipc->mutex_ipc.shell_list));
    /* 将互斥信号量插入shell列表 */
    mos_list_tail_insert(&g_mos_shell_ipc_list, &(to_creat_mutex_ipc->mutex_ipc.shell_list));
#endif	
