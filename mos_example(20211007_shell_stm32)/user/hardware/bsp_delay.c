 ...
 *	delay_xms()不会导致任务切换。delay_ms()其实就是对 MOS中的延时函数mos_task_delay()的
...
void delay_init()
{
    ...
    //reload = SystemCoreClock / MOS_CONFIG_CPU_FREQUENCY;	//根据configTICK_RATE_HZ设定溢出时间
    //reload为24位寄存器,最大值:16777216,在72M下,约合0.233s左右
    how_ms_per_rate = 1000 / MOS_CONFIG_CPU_FREQUENCY;		//代表OS可以延时的最少单位

    //SysTick->CTRL |= SysTick_CTRL_TICKINT_Msk;   	//开启SYSTICK中断
    //SysTick->LOAD = reload; 						//每1/MOS_CONFIG_CPU_FREQUENCY
